...
public class TournamentService : ITournamentService
{
    ...
    public async Task UpdateAsync(int id, Tournament tournament)
    {
        ...
        if (existing.Status != TournamentStatus.Pending)// si el torneo es diferente a pendiente envie el sguiente msj
        {
            throw new InvalidOperationException(
                "Solo se pueden editar torneos en estado Pending");
        }

        if (tournament.EndDate <= tournament.StartDate)// validacion de fecha. que un torneo no empiece antes de la fecha de ser creado
        {
            throw new InvalidOperationException(
                "La fecha de finalización debe ser posterior a la fecha de inicio");
        }
    ...
    public async Task UpdateStatusAsync(int id, TournamentStatus newStatus)
    {
        ...
        // Validar transiciones válidas; swtiche case abreviado, estoy validando qque la transicion de un estado a otro sea correcta 
        var validTransition = (tournament.Status, newStatus) switch
        {
            (TournamentStatus.Pending, TournamentStatus.InProgress) => true,// sino estan estas dos conbinaciones, entonces devuelvame un falso
    ...
    public async Task<IEnumerable<Team>> GetTeamsByTournamentAsync(int tournamentId)
    {
        ...
        // este select es un filtro que devuleve solo los equipos registrados en el torneo
    }
}
