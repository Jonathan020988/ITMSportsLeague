...
[ApiController]
[Route("api/[controller]")]// nombre url de este controlador
public class PlayerController : ControllerBase
{
    ...
    {
        var players = await _playerService.GetAllAsync();//lista de jugadores entidad
        var playersDto = _mapper.Map<IEnumerable<PlayerResponseDTO>>(players);//mapearla para q se vea dto
        return Ok(playersDto);// y luego se devuelve a lista
    }

    [HttpGet("{id}")]
    public async Task<ActionResult<PlayerResponseDTO>> GetById(int id)
    {
        var player = await _playerService.GetByIdAsync(id);// voy al servicio obtengo jugador por id
